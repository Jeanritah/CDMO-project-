# -*- coding: utf-8 -*-
"""sat_decision_sb.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZkpJTFuLigOyyjPHrz7U0QFPbX0mQT5M
"""
...
def solve_tournament_sat_safe_sb(n):
    ...
    # Team plays exactly one match/week
    for i in range(n):
        for w in range(W):
            s.add(exactly_one(
    ...
    # Team assigned to exactly one period per week
    for i in range(n):
        for w in range(W):
            s.add(exactly_one([per[i][w][p] for p in range(P)]))

    # Each period has exactly two teams
    for w in range(W):
        for p in range(P):
            s.add(exactly_k([per[i][w][p] for i in range(n)], 2))

    # At most two uses of each period per team
    for i in range(n):
        for p in range(P):
    ...
    # SB: Fix Team 1 vs Team n in Week 0: Team 1 is home
    s.add(home[0][n-1][0])

    #Solving
    t0 = time.time()
    res = s.check()
    runtime = int((time.time() - t0) // 1)
...
for n in instances:
    ...
    with open(f"res/SAT/{n}.json","w") as f:
        json.dump({"z3_sb_noopt":result}, f, indent=2)
    print("Saved.")